...
처음 실행할 때 Python 다운로드가 한 번 진행될 수 있습니다. 이후 명령도 같은 방식으로 `--python 3.12`를 붙여 실행합니다.
...
VS Code에서 직접 열 경우, 노트북 커널이 수업용 uv/Python 3.12 환경인지 확인합니다. Anaconda나 예전 전역 Python 커널이 선택되어 있으면 패키지 설치와 API 실행 결과가 달라질 수 있습니다.
...
```powershell
uv run --python 3.12 --with-requirements requirements.txt python test_examples.py
```
