...
1. `live_openai_smoke.py`로 OpenAI API 연결 확인. 정상 경로 기준 1회 호출.
2. `section1_llm_api.py`로 prompt와 live LLM 출력 확인. 정상 경로 기준 note 4개에 대해 약 4회 호출.
3. `section2_structured_output.py`로 Pydantic validation 확인. Section 1 결과를 다시 검증하므로 추가 호출 없음.
4. `section2_langchain_contract.py`로 LangChain prompt/parser 계약 확인. 정상 경로 기준 약 1회 호출.
5. `section3_rag.py`로 retrieval/evidence + live answer 확인. 기본 질문 수에 따라 약 3회 호출.
6. `section3_langchain_rag.py`로 LangChain 객체 확인. 기본 질문 수에 따라 약 3회 호출.
7. `section4_workflow.py`로 전체 흐름 확인. 기본 질문 수에 따라 약 2회 호출.
8. `test_examples.py`로 비용 없는 로컬 검사 확인. 이 파일은 LLM 품질 검사가 아니라 API 없이 잡을 수 있는 기본 규칙 검사입니다.
9. 마지막 확인이 필요할 때 `RUN_LIVE_OPENAI_TESTS=1`을 켠 뒤 `test_live_examples.py`로 live API 전체 검증. 일반 실습에서는 매번 실행하지 않습니다.

JSON 형식이 깨지면 일부 함수가 수정 요청을 한 번 더 보낼 수 있어 호출 수가 늘어날 수 있습니다. 비용이 걱정되면 노트북을 한 셀씩 실행하고, 실패한 셀만 다시 실행합니다.
